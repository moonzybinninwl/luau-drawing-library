...
getgenv().getrenderproperty = function(drawingObj, property)
    if type(drawingObj) ~= "table" and type(drawingObj) ~= "userdata" then
        error("getrenderproperty: expected drawingObj to be table or userdata, got " .. type(drawingObj), 2)
    end
    if type(property) ~= "string" then
        error("getrenderproperty: property name must be a string, got " .. type(property), 2)
    end
    local success, drawingProperty = pcall(function()
        return drawingObj[property]
    end)
    if not success then
        error("getrenderproperty: failed to get property '" .. property .. "': " .. tostring(drawingProperty), 2)
    end
    if drawingProperty == nil then
        error("getrenderproperty: property '" .. property .. "' is nil or does not exist", 2)
    end
